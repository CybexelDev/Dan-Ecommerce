...
const AddProductModal = ({ isOpen, onClose, onProductAdded }) => {
    ...
    // Update subcategories and IDs when category changes
    useEffect(() => {
        if (formData.category) {
            const selectedCategory = categories.find(cat => cat.category === formData.category);
            setSubCategories(selectedCategory?.subCategories || []);

            // Set categoryId and reset subcategory fields
            setFormData(prev => ({
                ...prev,
            ...
            setSubCategories([]);
            // Reset categoryId when no category selected
            setFormData(prev => ({
                ...prev,
                categoryId: '',
                subCategoryId: ''
    ...
    // Update subCategoryId when subcategory changes
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await addNewProduct(submitData);
            if(response?.data){
                // Call the callback function to add product to list
                if (onProductAdded) {
                ...
                <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-6">
                            ...
                            {/* Hidden debug info (optional - remove in production) */}
                            {/* <div className="text-xs text-gray-500 space-y-1">
                                <div>Category ID: {formData.categoryId || 'Not selected'}</div>
                                <div>SubCategory ID: {formData.subCategoryId || 'Not selected'}</div>
                            </div> */}
