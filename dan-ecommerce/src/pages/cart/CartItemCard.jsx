...
function CartItemCard( {id,name,brand,image,price,qty, onDelete}) {
  return (
    <div className='w-full aspect-[884/125]  flex justify-between items-center    border-b px-[1vw] pt-[1vw]'>
        ...
        <div className="w-1/3 h-full  flex items-center justify-between pr-[3vw]">
            {/* Minus Button */}
            <button className="h-[90%] aspect-square flex justify-center items-center pb-[.3vw] 
                                text-[1.5vw] font-bold border 
                                rounded-full hover:bg-gray-200 transition">
                -
            </button>

            {/* Quantity */}
            <span className=" text-[1.5vw] w-[3vw] h-[2vw] font-semibold flex justify-center items-center bg-[#D9D9D9] rounded-[.5vw]">{qty}</span>

            {/* Plus Button */}
            <button className=" h-[90%] aspect-square  flex justify-center items-center pb-[.3vw] 
                                text-[1.5vw] font-bold leading-none border 
                                rounded-full hover:bg-gray-200 transition">
                +
            </button>
            </div>
        ...
        <div className="w-1/3 h-full  flex justify-end px-[2vw]">
            <button onClick={() => onDelete(id)} className='text-[1.8vw]'>
                <RiDeleteBin6Fill />
            </button>
        </div>
        </div>
